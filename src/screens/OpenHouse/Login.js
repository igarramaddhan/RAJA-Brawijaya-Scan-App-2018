...
import { color, tokens, getLoginUKMUrl } from '../../libs/metrics';
...
class Login extends Component<Props, State> {
	...
	async login(username: String, password: String) {
		...
		try {
			let response = await fetch(getLoginUKMUrl(username, password));
			try {
				...
				if (responseJson.ukm) {
					try {
						await AsyncStorage.setItem(
							tokens.OPEN_HOUSE,
							JSON.stringify(responseJson.ukm)
						);
						this.props.store.setStoreState({
							dataUKM: responseJson.ukm
						});
		...
		return (
			...
			>
				<ScrollView contentContainerStyle={styles.container}>
					...
					>
						<Text style={styles.title}>Open House App</Text>
						<FormInput
